...
```
            */
            ...
            const response = await axiosInstance.post("auth/login", {
                ...credentials,
                expiresInMins: 24 * 60,
            }) as { accessToken: string }  // axiosInstance is an instance of the axios library that we have created to set a base URL for all our API requests and to manage common configurations such as headers and timeouts in one place.
            ...
            // we added the type assertion `as { data: { accessToken: string } }` to specify the expected structure of the response from the server. this is because we expect the response from the server to contain a data property which is an object that has an accessToken property of type string. by adding this type assertion, we can ensure that we are handling the response from the server correctly and that we are accessing the access token in a type-safe way in our code. this can help us to avoid errors and to improve the maintainability of our code when working with API responses in our application.
            // again, we modified the type assertion to `as { accessToken: string }` because we have already set up a response interceptor in our axios instance that returns the data property of the response object. this means that when we receive the response from the server, it will already be parsed and we can directly access the accessToken property without having to access it through the data property. by modifying the type assertion to `as { accessToken: string }`, we can ensure that we are handling the response from the server correctly and that we are accessing the access token in a type-safe way in our code. this can help us to avoid errors and to improve the maintainability of our code when working with API responses in our application.
            ...
            const loggedInUserInfo = await axiosInstance.get("auth/me") as { role: string }

...
const routerData = createBrowserRouter([
    ...
    // grouping admin routes together:
    {
        path: "/admin", element: <AdminLayout />, children: [   // This means that when the user navigates to any route that starts with "/admin", the AdminLayout element will be rendered. The children array defines the nested routes that will be rendered inside the AdminLayout component based on the specific path. This is working because of the <Outlet /> component used in the AdminDashboardMain component, which allows the nested routes to be rendered within the AdminLayout.
            ...
            { path: "users/:id/edit", element: <> User edit </> },  // this defines the route for editing a user under the "/admin" path which is already nested under the "/admin" path. This "users/:id/edit" paths says that when the user navigates to "/admin/users/:id/edit", the User edit component will be rendered inside the AdminLayout component, where it is inside AdminDashboardMain.tsx. The ":id" part is a route parameter that can be accessed in the User edit component to get the specific user id that needs to be edited. Do not use "/admin/users/:id/edit" here because it is already nested under the "/admin" path, so we can just use "users/:id/edit" to define the route for editing a user. Also, do not use "/users/:id/edit" because it will be treated as an absolute path and will not be nested under the "/admin" path.
